...
if chart_type == 'Gráfico de líneas':
    ...
    fig, ax = plt.subplots()
    data[['incoming_inner_count', 'incoming_outer_count']].head(96).plot.area(ax=ax,
    ...
    fig, ax = plt.subplots()
    plot = data[['Day','total_visits']].groupby('Day').sum().plot.bar(ax = ax, title = 'Visitas por día')
    st.pyplot(fig)

    #Gráfico de barras horizontal
    st.subheader('Gráfico de barras horizontal')
    fig, ax = plt.subplots()
    plot = data[['Day','total_visits']].groupby('Day').sum().plot.barh(ax = ax, title = 'Visitas por día')
    st.pyplot(fig)
elif chart_type == 'Gráfico de boxplot':
    #Gráfico de boxplot
    st.subheader('Gráfico de boxplot')
    fig, ax = plt.subplots()
    plot = data[['Day','total_visits']].head(7*96).plot.box(ax = ax, column = 'total_visits',title = 'Visitas por día')
    st.pyplot(fig)
elif chart_type == 'Gráfico de densidad':
    #Gráfico de densidad
    st.subheader('Gráfico de densidad')
    fig, ax = plt.subplots()
    plot = data[['incoming_inner_count','outgoing_inner_count']].head(96).plot.density(ax = ax)
    st.pyplot(fig)
elif chart_type == 'Gráfico de hexbin':
    #Gráfico de hexbin
    st.subheader('Gráfico de hexbin')
    fig, ax = plt.subplots()
    plot = data.head(96).plot.hexbin(ax = ax, x= 'incoming_outer_count',
    ...
    fig, ax = plt.subplots()
    plot = data[['incoming_inner_count','outgoing_inner_count']].plot.hist(ax= ax, title = 'Gráfico de histograma')
    st.pyplot(fig)
elif chart_type == 'Gráfico de line':
    #Gráfico de line
    st.subheader('Gráfico de line')
    fig, ax = plt.subplots()
    plot = data[['incoming_inner_count','outgoing_inner_count']].plot.line(ax= ax, title = 'Gráfico de lineas')
    st.pyplot(fig)
    ...
    fig, ax = plt.subplots()
    plot = data.plot.scatter(ax= ax, x = 'incoming_inner_count', y = 'outgoing_inner_count')
    st.pyplot(fig)

    st.subheader('Gráfico de scatter')
    fig, ax = plt.subplots()
    plot = data.plot.scatter(ax= ax, x = 'incoming_inner_count', y = 'outgoing_inner_count', c = 'total_visits')
    st.pyplot(fig)