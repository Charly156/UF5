...
if chart_type == 'Gráfico de líneas':
    ...
    fig, ax = plt.subplots()
    filtro_fecha[['Day', 'total_visits']].groupby(filtro_fecha['Day']).sum().plot(ax=ax, kind='line',
    ...
    fig, ax = plt.subplots()
    plot = filtro_fecha[['Day','total_visits']].groupby('Day').sum().plot.pie(ax= ax, y = 'total_visits', title = 'Visitas por día')
    st.pyplot(fig)
