...
chart_type = st.sidebar.selectbox('Selecciona el tipo de gráfico que desee visualizar', 
                          ['Gráfico de líneas', 'Gráfico de áreas', 'Gráfico de barras',
                           ...
                           'Gráfico de scatter'])
graph_color = st.sidebar.color_picker('Color')

col1, col2 = st.columns(2)
with col1:
    st.subheader('Gráfico de áreas')
    fig, ax = plt.subplots()
    filtro_fecha[['incoming_inner_count','incoming_outer_count']].head(96).plot.area(ax = ax, title = 'Gráfico de área', color = graph_color)
    st.pyplot(fig)
...
if chart_type == 'Gráfico de líneas':
    ...
    # Gráfico de áreas
    st.subheader('Gráfico de áreas')
    area_plot = px.area(filtro_fecha.head(96), x = 'timestamp', 
                        y = ['incoming_inner_count', 'incoming_outer_count'], 
                        title = 'Entradas en un día', 
                        color_discrete_sequence = px.colors.qualitative.Set1)
    #st.plotly_chart(area_plot)
    selected_point = plotly_events(area_plot, click_event = True)

    df_selected = None

    with st.expander('Desplegar datos del punto seleccionado', expanded = False):
        st.write('A continuación se muestra la tabla con los datos correspondientes al punto seleccionado')
    while :
    if len(selected_point) > 0:
        selected_x_value = selected_point[0]['x']
        selected_y_value = selected_point[0]['y']
        df_selected = filtro_fecha[

            (filtro_fecha['timestamp'] == selected_x_value)
            & (filtro_fecha['incoming_inner_count'] == selected_y_value)
        ]
